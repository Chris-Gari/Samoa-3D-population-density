...
florida_data <- st_read("data/kontur_population_US_20220630.gpkg")
...
florida <- st |> 
  filter(NAME == "Florida") |> 
  st_transform(crs = st_crs(florida_data))
...
st_florida <- st_intersection(florida_data, florida)
...
bottom_left <- st_point(c(bb[["xmin"]], bb[["ymin"]])) |> 
  st_sfc(crs = st_crs(florida_data))
