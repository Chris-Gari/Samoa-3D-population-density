...
```{r}
sm_hex <- st_read("data/kontur_topology_boundaries_WS_20230628.gpkg") %>% st_transform(3106)

sm_admin <- st_read("data/kontur_population_WS_20231101.gpkg") %>% st_transform(3106)
```
...
```{r}
#distinct_names <- sm_admin %>% distinct(name_en)
#print(distinct_names)
```

```{r}
# Creating BD Boundary
sm_boundary <- sm_admin %>%
  st_geometry %>%
  st_union %>%
  st_sf %>%
  st_make_valid()
...
ggplot(sm_admin) + 
  ...
  geom_sf(data = sm_boundary, fill = NA, color = "black")
```
...
```{r}
pop_matrix <- matrix(sm_admin$population,
                     nrow = floor(1574),
                     ncol = floor(1196))
```
